...
from commons import FEATURE_COLUMNS, MODEL_PATH
...
logger = logging.getLogger("heart-disease-prediction-service")
...
class HeartDiseaseRequest(BaseModel):
    """The 13 clinical model inputs in the same order as the training notebook."""
    age: int = Field(ge=1)
    gender: Literal[0, 1]
...
async def access_log_middleware(request: Request, call_next):
    ...
    logger.info(
        ...
        duration_ms,
        extra={
            "event": "prediction_request" if request.url.path == "/predict" else "http_request",
...
def predict(request: HeartDiseaseRequest):
    with tracer.start_as_current_span("model_inference") as span:
        ...
        input_df = pd.DataFrame([request.model_dump()], columns=FEATURE_COLUMNS)

        prediction = model.predict(input_df)[0]

        response = {
            "predicted_class": str(prediction),
            "has_heart_disease": str(prediction) == "yes",
            "span_id": span_id,
        }
